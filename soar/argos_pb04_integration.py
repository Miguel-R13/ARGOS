    ...
    try:
        for line in proc.stdout:
            ...
            if alert:
                pid = alert.get('pid', '')
                now_ts = time.time()
                if pid and pid in processed_pids and (now_ts - processed_pids[pid]) < 30:
                    log(f"Deduplicacion: PID {pid} ya procesado hace {int(now_ts - processed_pids[pid])}s - ignorando")
                    continue
                if pid:
                    processed_pids[pid] = now_ts
