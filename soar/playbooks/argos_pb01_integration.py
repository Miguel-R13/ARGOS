#!/usr/bin/env python3
"""
ARGOS SOC - PB01 Reverse Shell / C2 - Integration Script (servidor .10)
Monitoriza alerts.json en tiempo real, detecta rule 100601 (reverse shell confirmada)
y ejecuta contencion activa via SSH en el endpoint afectado.

Flujo:
  1. Lee alerts.json en tiempo real (tail -f)
  2. Filtra alertas de rule 100601 desde agente 002 (ARGOS-Endpoint-Linux)
  3. Extrae IP de C2 y PID del proceso desde el JSON
  4. Conecta via SSH al endpoint afectado
  5. Ejecuta ufw deny (bloqueo IP C2) y kill -9 (terminar proceso)
  6. Notifica resultado a grupo Telegram ARGOS SOC Alerts

Ejecucion: servidor Wazuh (.10) como servicio o proceso en background
...
# Mapeo agent_id -> IP del endpoint
AGENT_IPS = {
    "002": "192.168.234.30",
}

# Rule IDs que disparan PB01
TRIGGER_RULES = {"100601"}

LOG_FILE = "/var/ossec/logs/argos_pb01_integration.log"


def log(message):
    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    entry = f"[{timestamp}] PB01-INTEGRATION | {message}"
...
def block_ip(endpoint_ip, c2_ip):
    cmd = f"sudo ufw deny from {c2_ip} to any"
    ok, out, err = ssh_execute(endpoint_ip, cmd)
    if ok:
        log(f"UFW: IP {c2_ip} bloqueada en {endpoint_ip}")
    else:
        log(f"UFW: ERROR bloqueando {c2_ip} en {endpoint_ip} - {err}")
    return ok
...
def parse_alert(line):
    try:
        ...
        match = re.search(r"laddr=([\d.]+)", full_log)
        if match:
            c2_ip = match.group(1)
...
def respond(alert):
    ...
    if alert["c2_ip"]:
        ...
        if ok:
            acciones.append(f"IP bloqueada: <code>{alert['c2_ip']}</code>")
        else:
            errores.append(f"Fallo al bloquear IP: {alert['c2_ip']}")
    else:
        errores.append("IP de C2 no disponible en la alerta")
    ...
    mensaje = (
        f"{emoji} <b>ARGOS SOC - PB01 REVERSE SHELL DETECTADA</b>\n\n"
...
def main():
    log("=" * 60)
    log("PB01 Integration iniciado - Monitorizando alerts.json")
    ...
    try:
        ...
        log("PB01 Integration detenido por el usuario.")
        proc.terminate()
